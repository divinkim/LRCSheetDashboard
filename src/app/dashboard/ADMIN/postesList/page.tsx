...
export default function PostsList() {
    ...
    const { postsListCloned, start, maxPage, onSearch, requireAdminRoles, getAdminRole, limit, setPage, page } = PostsListHook();
                ...
                <main className='m-4 bg-gray-100 font-semibold text-gray-700 dark:text-gray-300 dark:bg-transparent'>
                    {
                        tablesModal.map((e) => (
                            <div className="flex justify-between items-center">
                                <h1 className="text-[20px] my-4 font-bold dark:text-gray-300">{e.postsList.pageTitle}  </h1>
                                <p className='text-blue-700 dark:text-blue-600 hidden xl:block'>{e.postsList.path}</p>
                            </div>
                        ))
                    }
                    ...
                    <table className="border w-full mx-auto">
                        <thead>
                            <tr className="bg-gray-800 dark:bg-transparent ">
                                {
                                    tablesModal.map((e) => (
                                        e.postsList.table.titles.map((item) => (
                        ...
                        <tbody className="w-full">
                            ...
                            {
                                postsListCloned.length > 0 ? postsListCloned.slice(start, start + limit).map((u) => (
                                    <tr className="">
                                        <td className="border p-2 border-gray-400 dark:border-gray-300  text-center font-semibold dark:text-gray-300">{u.title}</td>
                                        <td className="border p-2 border-gray-400 dark:border-gray-300  text-center font-semibold dark:text-gray-300">{u.description}</td>
                                        ...
                                        <td className="border p-2 border-gray-400 dark:border-gray-300  text-center font-semibold dark:text-gray-300">{u?.DepartmentPost.name}</td>
                                        <td className="text-center py-5 font-semibold border-b border-r  space-x-3 flex justify-center h-auto p-2 border-gray-400 dark:border-gray-300">
                                            <button className="bg-gray-300 hover:scale-105 ease duration-500 p-2 rounded-md" onClick={() => {
                                                ...
                                                <Link href={`/dashboard/ADMIN/updatePost/${u.id}`} >
