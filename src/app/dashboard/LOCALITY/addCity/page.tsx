...
import { AddCityHook } from "./hook";
import { formElements } from "@/components/FormElements/forms";
import { ClipLoader } from "react-spinners";

export default function AddCity({ isVisibleForm = false }) {
    const { inputs, setInputs, handleSubmit, dynamicArrayDatas, isLoading } = AddCityHook();

    return (
        <div className={isVisibleForm ? "w-full h-full flex items-center justify-center fixed bg-black/50" : "hidden"}>
            <div className="p-4 w-1/2 xl:w-[45%] bg-white dark:bg-gray-800  rounded-md  relative right-[140px] -top-14">
                <div className="flex justify-between items-center">
                    <h1 className="font-bold text-[18px]">{formElements[0].addCity.titleForm}</h1>
                ...
                <div className="grid grid-cols-1 mb-4  gap-4">
                    {
                        formElements[0].addCity.inputs.map((item, index) => (
                                        ...
                                        {item.label}</label>
                                    {
                                        !item.selectedInput ?
                                            <input onChange={async (u) => {
                                                ...
                                                localStorage.setItem("inputMemoryO.Page", JSON.stringify(fieldValue))
