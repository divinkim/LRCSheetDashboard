...
export function AddQuarterHook() {
    ...
    async function handleSubmit() {
        ...
        const fielControle = controllers.verifyRequireField(inputs);

        if (!fielControle.status) {
            setTimeout(() => {
                fielControle.message;
                setIsLoading(false);
            }, 1000)
            return
        }

        const response = await controllers.API.SendOne(urlAPI, "createQuarter", null, inputs);
        controllers.alertMessage(response.status,
            response.title, response.message,
            response.status ? "/dashboard/Localities/quartersList" : null
        )
        setIsLoading(false);
    }
    ...
    let dynamicArrayDatas = [
        ...
        {
            alias: "DistrictId",
            arrayData: districts.filter((item: { id: number, name: string }) => item.id && item.name).map((item: { id: number, name: string }) => ({ value: item.id, title: item.name }))
        },
