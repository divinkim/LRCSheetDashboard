...
export default function AddQuarter({ isVisibleFormQuarter = false }) {
    const { inputs, setInputs, handleSubmit, dynamicArrayDatas, isLoading } = AddQuarterHook();

    return (
        <div className={isVisibleFormQuarter ? "w-full h-full flex items-center justify-center fixed bg-black/50" : "hidden"}>
            <div className="p-4 w-1/2 bg-white dark:bg-gray-800  rounded-md  relative right-[140px] -top-14">
                ...
                <div className="grid grid-cols-1 mb-4 lg:grid-cols-2 gap-4">
                    {
                        formElements[0].addQuarter.inputs.map((item, index) => (
                                        ...
                                        {item.label}</label>
                                    {
                                        !item.selectedInput ?
                                            <input onChange={async (u) => {
                                                ...
                                                localStorage.setItem("inputMemoryOfQuarterPage", JSON.stringify(fieldValue))
