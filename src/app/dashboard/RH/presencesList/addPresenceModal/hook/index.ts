...
export default function useAddPresenceModal() {
    ...
    const handleSubmit = async () => {
        ...
        if (!inputs.arrivalTime || inputs.usersId.length < 0) {
            setTimeout(() => {
                setIsLoading(false);
                controllers.alertMessage(false, "Champs invalides", "Veuillez sélectionner au moins un utilisateur et une heure d'arrivée", null);
            }, 1000);
            return;
        }
        ...
        if (response.status) {
            ...
            controllers.alertMessage(response.status, response.title, response.message, "/dashboard/RH/presencesList");
            return;
        }
